		...
		<div class="panel panel-default">
			...
			<div class="panel-body">
				<table width="100%"
					...
					<c:forEach items="${list }" var="subject">
						<tr class="odd gradeX">
							...
							<td>
								<a class='move' href=<c:out value="${subject.bno}"/>>
								<c:out value="${subject.hospitalsub}"/></a>
							</td>
							
							<td>
								<a href=<c:out value="${subject.symptom}"/>>
								<c:out value="${subject.symptom }" /></a>
							</td>
						</tr>
					</c:forEach>
				</table>
				...
				<!-- 검색 조건 Start -->
				 ...
				 <%-- <form id="searchForm" action="/subject/list" method="get">
					<select name='type'>
						<option value="" <c:out value="${pageMaker.cri.type==null ? 'selected' : ''}"/> ></option>
						<option value="H" <c:out value="${pageMaker.cri.type eq 'H' ? 'selected' : ''}"/>>과목</option>
						<option value="S" <c:out value="${pageMaker.cri.type eq 'S' ? 'selected' : ''}"/>>주요 진로 증상</option>
					</select>
...
<!-- 		var actionForm = $("#actionForm")
		$(".paginate_button a").on("click", function(e) {
			...
			actionForm.submit();
				...
				<form id="actionForm" action="/subject/list" method="get">
...
<script>
	$(document).ready(function() {
		...
		$("#regBtn").on("click",function(){
			self.location = "/subject/register";
		});
	...
	/* 	 	actionForm.append("<input type='hidden' name='bno' value='"+$(this).attr("href")"'>"); => 오류 */
			...
			actionForm.attr("action", "/subject/get");
			actionForm.submit();
			})
